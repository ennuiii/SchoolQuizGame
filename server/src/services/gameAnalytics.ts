...
import { GameAnalytics, GameStats, GameFinalStats, Player, PlayerStats } from '../types';
...
class GameAnalyticsService implements GameAnalytics {
  ...
  addGame(roomCode: string): void {
    logger.log(`Creating analytics for new game in room ${roomCode}`);
    this.games[roomCode] = {
      startTime: new Date(),
      players: [],
      rounds: [],
  ...
  addPlayer(roomCode: string, player: Player): void {
    if (!this.games[roomCode]) {
      logger.warn(`Tried to add player to non-existent game analytics for room ${roomCode}`);
      return;
    }
    
    logger.log(`Adding player ${player.name} to game analytics for room ${roomCode}`);
  ...
  recordAnswer(roomCode: string, playerId: string, answer: string, isCorrect: boolean | null, responseTime: number): void {
    ...
    if (!game) {
      logger.warn(`Tried to record answer for non-existent game analytics in room ${roomCode}`);
      return;
    }
    
    const player = game.players.find(p => p.id === playerId);
    if (!player) {
      logger.warn(`Player ${playerId} not found in game analytics for room ${roomCode}`);
      return;
    }
    ...
    logger.log(`Recorded answer for player ${playerId} in room ${roomCode}`, { 
      isCorrect, 
      responseTime 
    });
  }
  ...
  endGame(roomCode: string): GameFinalStats | undefined {
    const game = this.games[roomCode];
    if (!game) {
      logger.warn(`Tried to end non-existent game analytics for room ${roomCode}`);
      return;
    }
    ...
    if (totalPlayers === 0 || game.totalQuestions === 0) {
      logger.warn(`Cannot calculate final stats for room ${roomCode}: no players or questions`);
      return;
    }
    ...
    const averageScore = game.players.reduce((sum, p) => 
      sum + (p.correctAnswers / Math.max(1, game.totalQuestions)), 0) / totalPlayers;
    
    const finalStats: GameFinalStats = {
      totalPlayers,
      averageScore,
      fastestPlayer,
      mostAccuratePlayer
    ...
    logger.log(`Game ended in room ${roomCode}, final stats calculated`);
  ...
  private findFastestPlayer(players: PlayerStats[]): PlayerStats | null {
    if (players.length === 0) return null;
    
    return players.reduce((fastest, player) => {
      // Only consider players with at least one answer
      if (player.answers.length === 0) return fastest;
  ...
  private findMostAccuratePlayer(players: PlayerStats[], totalQuestions: number): (PlayerStats & { accuracy: number }) | null {
    if (players.length === 0 || totalQuestions === 0) return null;
    
    let mostAccurate: (PlayerStats & { accuracy: number }) | null = null;
