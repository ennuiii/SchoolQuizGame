...
function checkAllPlayersLostLives(roomCode: string): boolean {
  const room = gameRooms[roomCode];
...
function getAffectedPlayersForRestore(roomCode: string): Array<{ persistentPlayerId: string, name: string, livesLost: number }> {
  const room = gameRooms[roomCode];
...
function restoreRoundLives(roomCode: string): Array<{ persistentPlayerId: string, name: string, livesRestored: number }> {
  const room = gameRooms[roomCode];
  if (!room || !room.roundStartLives) {
    return [];
  }
...
function checkAndOfferLifeRestore(roomCode: string): void {
  const room = gameRooms[roomCode];
