...
app.use((0, cors_1.default)({
    origin: function (origin, callback) {
        ...
        if (allowedOrigins.indexOf(origin) === -1) {
            logService_1.logger.log('CORS blocked origin:', origin);
...
try {
    ...
    logService_1.logger.error('Failed to load room state on startup:', error);
}
        ...
        return {
            ...
            submissions: room.players.map(player => {
                ...
                if (hasDrawing && (!drawingData || drawingData.trim() === '')) {
                    logService_1.logger.log(`Player ${player.persistentPlayerId}, Round ${index + 1}: Drawing data missing in answer. Checking boardsForRound.`, {});
                    ...
                    if (boardsForRound[player.persistentPlayerId]) {
                        ...
                        logService_1.logger.log(`Player ${player.persistentPlayerId}, Round ${index + 1}: Found drawing data in boardsForRound. Length: ${drawingData?.length || 0}`, {});
                    }
                    // Also try to find using player.id as a fallback
                    else if (boardsForRound[player.id]) {
                        drawingData = boardsForRound[player.id];
                        logService_1.logger.log(`Player ${player.persistentPlayerId}, Round ${index + 1}: Found drawing data in boardsForRound using socket ID. Length: ${drawingData?.length || 0}`, {});
                    }
                    // If we still don't have drawing data, set hasDrawing to false
                    if (!drawingData || drawingData.trim() === '') {
                        logService_1.logger.warn(`Player ${player.persistentPlayerId}, Round ${index + 1}: No drawing data found in boardsForRound. Setting hasDrawing to false.`, {});
                        hasDrawing = false;
                    }
                }
                return {
...
if (process.env.NODE_ENV === 'production') {
    ...
    for (const pathToCheck of possibleBuildPaths) {
        if (fs_1.default.existsSync(pathToCheck)) {
            ...
            logService_1.logger.log(`Using build path: ${buildPath}`, {});
            logService_1.logger.log(`Build index.html exists: ${fs_1.default.existsSync(path_1.default.join(buildPath, 'index.html'))}`, {});
            buildPathFound = true;
            break;
        }
    }
    if (!buildPathFound) {
        logService_1.logger.error('No valid build path found. Checked these paths:', {});
        possibleBuildPaths.forEach(p => logService_1.logger.error(`- ${p} (exists: ${fs_1.default.existsSync(p)})`, {}));
        logService_1.logger.log(`Current directory: ${process.cwd()}`, {});
        logService_1.logger.log(`Directory listing for current directory: ${fs_1.default.readdirSync(process.cwd())}`, {});
        try {
            logService_1.logger.log(`Directory listing for server directory: ${fs_1.default.readdirSync(__dirname)}`, {});
        }
        catch (err) {
            logService_1.logger.error('Error reading server directory:', err);
        }
    }
...
io.use((socket, next) => {
    try {
        ...
        logService_1.logger.log(`Socket ${socket.id} authentication check:`, {
            query,
            isInitialConnection,
            auth
        });
        ...
        logService_1.logger.log(`Socket ${socket.id} authenticated:`, {
        ...
        logService_1.logger.error('Middleware error:', error);
        next(error);
    }
});
...
function finalizeRoundAndAutoSubmit(roomCode) {
    const room = roomService.getRoom(roomCode);
    if (!room) {
        logService_1.logger.log(`[FinalizeRound] Room ${roomCode} not found.`, {});
        return;
    }
    logService_1.logger.log(`[FinalizeRound] Finalizing round for room ${roomCode}. Current question index: ${room.currentQuestionIndex}`, {});
    ...
    if (room.players && room.currentQuestionIndex !== undefined && room.currentQuestionIndex !== null) {
        room.players.forEach(playerInRoom => {
            if (playerInRoom.isActive &&
                ...
                logService_1.logger.log(`[FinalizeRound] Auto-submitting for player ${playerInRoom.id} in room ${roomCode}`, {});
        ...
        logService_1.logger.warn(`[FinalizeRound] Could not perform auto-submissions for room ${roomCode}. Conditions not met: players array exists: ${!!room.players}, currentQuestionIndex defined: ${room.currentQuestionIndex !== undefined && room.currentQuestionIndex !== null}`, {});
    }
    broadcastGameState(roomCode);
    logService_1.logger.log(`[FinalizeRound] Game state broadcasted for room ${roomCode} after finalization.`, {});
}
// Helper function to conclude game and send recap to all
function concludeGameAndSendRecap(roomCode, winnerInfo = null) {
    const room = roomService.getRoom(roomCode);
    if (!room) {
        logService_1.logger.log(`[ConcludeGame] Room ${roomCode} not found. Skipping.`, {});
        return;
    }
    if (room.isConcluded) {
        logService_1.logger.log(`[ConcludeGame] Room ${roomCode} already concluded. Skipping recap send.`, {});
        return;
    }
    room.isConcluded = true;
    clearRoomTimer(roomCode); // Stop any active timers
    logService_1.logger.log(`[ConcludeGame] Game concluded in room ${roomCode}. Emitting game_over_pending_recap.`, {});
    ...
    if (recap) {
        logService_1.logger.log(`[ConcludeGame] Automatically broadcasting recap for room ${roomCode} with initialSelectedRoundIndex and initialSelectedTabKey.`, {});
        ...
        logService_1.logger.warn(`[ConcludeGame] Recap data generation failed for room ${roomCode} during auto-send.`, {});
    }
}
...
function startQuestionTimer(roomCode) {
    ...
    if (!room || !room.timeLimit) {
        logService_1.logger.log(`Cannot start timer for room ${roomCode}: room not found or no time limit.`, {});
        return;
    }
    // Get time limit in milliseconds
    const timerDurationMs = room.timeLimit * 1000;
    logService_1.logger.log(`Starting timer for room ${roomCode} with duration ${room.timeLimit} seconds.`, {});
    // Set timer
    const timer = setTimeout(() => {
        logService_1.logger.log(`Timer finished for room ${roomCode}. Auto-finalizing round.`, {});
        // Update room
        if (room) {
...
function clearRoomTimer(roomCode) {
    ...
    if (timer) {
        ...
        logService_1.logger.log(`Timer cleared for room ${roomCode}`, {});
    }
}
...
server.listen(PORT, () => {
    logService_1.logger.log(`Server running on port ${PORT}`, {});
});
// Initialize SocketIO service
(0, socketService_1.createSocketService)(server, allowedOrigins);
